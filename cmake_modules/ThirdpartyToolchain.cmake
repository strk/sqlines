...
if (NOT SQLINES_VERBOSE_THIRDPARTY_BUILD)
  set(EP_LOG_OPTIONS
    LOG_CONFIGURE 1
    LOG_BUILD 1
    LOG_INSTALL 1
...
if (NOT "$ENV{SQLINES_BUILD_TOOLCHAIN}" STREQUAL "")
  set(FLATBUFFERS_HOME "$ENV{SQLINES_BUILD_TOOLCHAIN}")
  set(RAPIDJSON_HOME "$ENV{SQLINES_BUILD_TOOLCHAIN}")
  set(JEMALLOC_HOME "$ENV{SQLINES_BUILD_TOOLCHAIN}")
  set(GFLAGS_HOME "$ENV{SQLINES_BUILD_TOOLCHAIN}")
  set(SNAPPY_HOME "$ENV{SQLINES_BUILD_TOOLCHAIN}")
  set(ZLIB_HOME "$ENV{SQLINES_BUILD_TOOLCHAIN}")
  set(BROTLI_HOME "$ENV{SQLINES_BUILD_TOOLCHAIN}")
  set(LZ4_HOME "$ENV{SQLINES_BUILD_TOOLCHAIN}")
  set(ZSTD_HOME "$ENV{SQLINES_BUILD_TOOLCHAIN}")

  if (NOT DEFINED ENV{BOOST_ROOT})
    # Since we have to set this in the environment, we check whether
    # $BOOST_ROOT is defined inside here
    set(ENV{BOOST_ROOT} "$ENV{SQLINES_BUILD_TOOLCHAIN}")
  endif()
endif()
...
if (MSVC AND SQLINES_USE_STATIC_CRT)
...
if (SQLINES_BOOST_VENDORED)
  ...
  add_dependencies(sqlines_dependencies boost_ep)
else()
  if (MSVC)
    # disable autolinking in boost
    add_definitions(-DBOOST_ALL_NO_LIB)
  endif()
  if (SQLINES_BOOST_USE_SHARED)
...
if (NOT SQLINES_BOOST_HEADER_ONLY)
  ...
  SET(SQLINES_BOOST_LIBS boost_system boost_filesystem boost_regex)
endif()
...
if(SQLINES_BUILD_TESTS OR SQLINES_BUILD_BENCHMARKS)
  ...
  if("$ENV{GTEST_HOME}" STREQUAL "")
    ...
    if (MSVC AND NOT SQLINES_USE_STATIC_CRT)
...
if (SQLINES_IPC)
  ...
  if(RAPIDJSON_VENDORED)
    add_dependencies(sqlines_dependencies rapidjson_ep)
  endif()
  ...
  if(FLATBUFFERS_VENDORED)
    add_dependencies(sqlines_dependencies flatbuffers_ep)
  endif()
...
if (SQLINES_JEMALLOC)
  ...
  set(SQLINES_JEMALLOC_USE_SHARED OFF)
  ...
  ExternalProject_Add(jemalloc_ep
    ...
    CONFIGURE_COMMAND ./autogen.sh "--prefix=${JEMALLOC_PREFIX}" "--with-jemalloc-prefix=je_sqlines_" "--with-private-namespace=je_sqlines_private_" "--disable-tls"
    ${EP_LOG_OPTIONS}
    BUILD_IN_SOURCE 1
    BUILD_COMMAND ${MAKE}
...
## near definition of SQLINES_USING_GOLD).
# find_package(GPerf REQUIRED)
# if (NOT "${SQLINES_USE_ASAN}" AND
#     NOT "${SQLINES_USE_TSAN}" AND
#     NOT ("${SQLINES_USING_GOLD}" AND "${SQLINES_LINK}" STREQUAL "d"))
...
#   list(APPEND SQLINES_BASE_LIBS tcmalloc profiler)
#   add_definitions("-DTCMALLOC_ENABLED")
#   set(SQLINES_TCMALLOC_AVAILABLE 1)
# endif()
